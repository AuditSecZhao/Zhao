...
sub linux_snmp
{
	...
	if($memtotal =~ /^[\d\.]+$/ && $memavail =~ /^[\d\.]+$/ && $memcache =~ /^[\d\.]+$/ && $membuff =~ /^[\d\.]+$/)
	{
	...
	if($swaptotal =~ /^[\d\.]+$/ && $swapavail =~ /^[\d\.]+$/ && $swaptotal > 0)
	{
...
sub warning_func
{
	...
	if(defined $disk)
	{
		$cmd = "select mail_alarm,sms_alarm,highvalue,lowvalue,unix_timestamp(mail_last_sendtime),unix_timestamp(sms_last_sendtime),send_interval from snmp_status where device_ip = '$device_ip' and type = 'disk' and disk = '$disk'";
	}
	else
	{
		$cmd = "select mail_alarm,sms_alarm,highvalue,lowvalue,unix_timestamp(mail_last_sendtime),unix_timestamp(sms_last_sendtime),send_interval from snmp_status where device_ip = '$device_ip' and type = '$type'";
	}
...
#		if(($monitor==1 && ($device_type==2 || $device_type==4 || $device_type==20 || $device_type==30)) || $monitor==2 || $monitor==3)
	...
	$sqr_select = $dbh->prepare("select snmp_status.device_ip,type,disk,servers.monitor from snmp_status left join servers on snmp_status.device_ip = servers.device_ip where snmp_status.datetime < '$time_now_str' and servers.monitor!=0");
	$sqr_select->execute();
	while(my $ref_select = $sqr_select->fetchrow_hashref())
	{
		my $device_ip = $ref_select->{"device_ip"};
		...
		if($type ne "disk")
		{
			...
			my $tmp_status = &warning_func($dbh,$time_now_str,$monitor,$device_ip,-100,$type,undef);
			if(($ip_status{$device_ip} == 1 && $tmp_status != 1) || ($ip_status{$device_ip} != 0 && $tmp_status == 0))
			{
				$ip_status{$device_ip} = $tmp_status;
			}
		...
		else
		{
			...
			my $tmp_status = &warning_func($dbh,$time_now_str,$monitor,$device_ip,-100,'disk',$disk);
			if(($ip_status{$device_ip} == 1 && $tmp_status != 1) || ($ip_status{$device_ip} != 0 && $tmp_status == 0))
			{
				$ip_status{$device_ip} = $tmp_status;
			}
...
#		判断 有没 进程 未写入 process 表 有的话 告警 
#		但由于 具体的进程仍旧未知 所以不会更新表项 只会告警  
#		目前不需要
...
#	当一个主机的所有 进程 都取不到值时 需要更改告警的内容
#	目前不需要
	$sqr_select = $dbh->prepare("select snmp_check_process.device_ip from snmp_check_process left join servers on snmp_check_process.device_ip = servers.device_ip where servers.monitor=3 group by device_ip HAVING max(process) < 0");
	$sqr_select->execute();
	while(my $ref_select = $sqr_select->fetchrow_hashref())
	{
#				/////////////// empty //////////////////
...
#		判断 有没 端口 未写入 port 表 有的话 告警 
#		但由于 具体的端口仍旧未知 所以不会更新表项 只会告警  
#		目前不需要
...
#				/////////////// empty //////////////////
	...
	while(my $ref_select = $sqr_select->fetchrow_hashref())
	{
		...
		if($debug == 1)
		{
			print "主机 $device_ip 进程: $port 没有取到\n";
		}
	}
	$sqr_select->finish();

#	当一个主机的所有 端口 都取不到值时 需要更改告警的内容
#	目前不需要
	$sqr_select = $dbh->prepare("select snmp_check_port.device_ip from snmp_check_port left join servers on snmp_check_port.device_ip = servers.device_ip where servers.monitor=3 group by device_ip HAVING max(snmp_check_port.port) < 0");
	$sqr_select->execute();
	while(my $ref_select = $sqr_select->fetchrow_hashref())
	{
#				/////////////// empty //////////////////
