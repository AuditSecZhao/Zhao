...
sub log_process
{
    ...
    foreach my $msg(<$fd_fr>)
    {
        ...
        my($warning,$mail_alarm,$sms_alarm,$instruction) = &is_warning($host,$msg);
        if($warning == 1)
        {
...
sub init_filter
{
    my $sqr_select = $dbh->prepare("select host,msg from log_syslog where process = 1");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        ...
        unless(defined $filter_list{$host})
        {
            my @tmp;
            $filter_list{$host} = \@tmp;
        }

        push @{$filter_list{$host}}, $msg;
    }
    $sqr_select->finish();
}
...
sub init_warning
{
    my $sqr_select = $dbh->prepare("select host,msg,instruction,mail_alarm,sms_alarm from log_syslog where process = 2");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        ...
        unless(defined $warning_list{$host})
        {
            my %tmp;
            $warning_list{$host} = \%tmp;
        }

        unless(exists $warning_list{$host}->{$msg})
        {
            my @tmp=($mail_alarm,$sms_alarm,$instruction);
            $warning_list{$host}->{$msg} = \@tmp;
        }
    }
    $sqr_select->finish();
}
...
sub is_filter
{
    ...
    my $msg_arr;
    if(exists $filter_list{$host})
    {
        $msg_arr = $filter_list{$host};
    }
    elsif(exists $filter_list{"NULL"})
    {
        $msg_arr = $filter_list{"NULL"};
    }
    else
    {
        return 0;
    ...
    if($msg_arr->[0] eq "NULL")
    {
        return 1;
    }
	else
	{
		foreach my $regex(@$msg_arr)
		{
			if($msg =~ /$regex/)
            {
                return 1;
...
sub is_warning
{
    ...
    my $msg_ref;
	if(exists $warning_list{$host})
    {
        $msg_ref = $warning_list{$host};
    }
	elsif(exists $warning_list{"NULL"})
    {
        $msg_ref = $warning_list{"NULL"};
    }
	else
    {
        return 0;
	...
	{
		foreach my $regex(keys %{$msg_ref})
		{
			if($msg =~ /$regex/)
            {
                my $ref = $msg_ref->{$regex};
                return (1,$ref->[0],$ref->[1],$ref->[2]);
            }
		}
		return (0,0,0,undef);
