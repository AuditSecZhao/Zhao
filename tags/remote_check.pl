...
sub remote_check_process
{
    ...
    my @output = &get_output($exp,$cmd,$regex);

    foreach my $line(@output)
    {
        print $fd_fw "$line\n";

        if($action eq 'c' && $line =~ /$regex/i)
        {
            $result = "OK";
            last;
        }

        if($action eq 'd' && $line =~ /$regex/i)
        {
            $result = $1;
            last;
        }
