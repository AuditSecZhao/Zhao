...
sub fork_process
{
	...
	if ($pid == 0)
	{
        ...
        foreach my $app_name(keys %{$device_info{$device_ip}->{$port}})
        {
            if($app_name eq 'apache')
            {
                &apache_process($dbh,$device_ip,$port,$app_name,$device_info{$device_ip}->{$port}->{$app_name});
            }
            elsif($app_name eq 'mysql')
            {
                &mysql_process($dbh,$device_ip,$port,$app_name,$device_info{$device_ip}->{$port}->{$app_name});
            }
            elsif($app_name eq 'tomcat')
            {
                &tomcat_process($dbh,$device_ip,$port,$app_name,$device_info{$device_ip}->{$port}->{$app_name});
            }
            elsif($app_name eq 'nginx')
            {
                &nginx_process($dbh,$device_ip,$port,$app_name,$device_info{$device_ip}->{$port}->{$app_name});
            }
...
sub apache_process
{
    my($dbh,$device_ip,$port,$app_name,$ref_arr) = @_;
...
sub nginx_process
{
    ...
    my($connect,$request) = &get_nginx_status($device_ip,$port);
    ...
    else
    {
        ...
        &warning_func($dbh,$device_ip,$port,'nginx','connect_num',-100);
