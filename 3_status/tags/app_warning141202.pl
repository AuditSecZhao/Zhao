...
our $dbh=DBI->connect("DBI:mysql:database=audit_sec;host=localhost;mysql_connect_timeout=3600","root","",{RaiseError=>1});
...
sub mail_alarm_process
{
    ...
    my $sqr_select = $dbh->prepare("select id,device_ip,datetime,app_name,app_type,cur_val,context from app_warning_log where mail_status = -1 order by datetime desc");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        ...
        if(defined $last_datetime && $last_datetime ne $datetime)
        {
            foreach my $deviceIp(keys %app_errlog)
            {
                ...
                foreach my $appName(keys %{$app_errlog{$deviceIp}})
                {
                    $device_errlog{$deviceIp} .= "应用名: $appName\n";
                    $device_errlog{$deviceIp} .= "$app_errlog{$deviceIp}->{$appName}";

                    if(scalar @{$app_err_cache{$deviceIp}->{$appName}} != 0)
                    {
                        $device_errlog{$deviceIp} .= join(",",@{$app_err_cache{$deviceIp}->{$appName}});
            ...
            foreach my $deviceIp(keys %app_err_cache)
            {
                ...
                foreach my $appName(keys %{$app_err_cache{$deviceIp}})
                {
                    $device_errlog{$deviceIp} .= "应用名: $appName\n";
        ...
        unless(exists $app_errlog{$device_ip}->{$app_name})
        {
            $app_errlog{$device_ip}->{$app_name} = "";
        }
        ...
        unless(exists $app_err_cache{$device_ip}->{$app_name})
        {
            my @tmp;
            $app_err_cache{$device_ip}->{$app_name} = \@tmp;
        }

        if($value < 0)
        {
            push @{$app_err_cache{$device_ip}->{$app_name}},$app_type
        }
        else
        {
            $app_errlog{$device_ip}->{$app_name} .= "$app_type $context\n";
        }
    }
    $sqr_select->finish();
...
#       my $status = &send_mail('wxl890206@126.com',$subject,$email_errlog{$mailto}->[1],$mailserver,$mailfrom,$mailpwd);
    ...
    foreach my $device_ip(keys %device_status)
    {
        ...
        foreach my $id(@{$device_status{$device_ip}->[0]})
        {
            ...
            if($status == 1)
            {
                my $sqr_select_app = $dbh->prepare("select app_name,app_type from app_warning_log where id = $id");
                ...
                my $sqr_select_seq = $dbh->prepare("select seq from app_status where device_ip = '$device_ip' and app_name = '$tmp_app_name' and app_type = '$tmp_app_type'");
...
sub sms_alarm_process
{
    ...
    my $sqr_select = $dbh->prepare("select id,device_ip,datetime,app_name,app_type,cur_val,context from app_warning_log where sms_status = -1 order by datetime desc");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        my $id = $ref_select->{"id"};
    ...
    {
        my $is_exist = 0;
        $sqr_select = $dbh->prepare("select mobilenum from member where uid IN(select memberid from snmp_alert_user a left join snmp_alert b on a.snmp_alert_id=b.seq where b.groupid=(select groupid from servers where device_ip='$device_ip') and b.enable=1) and mobilenum != ''");
        $sqr_select->execute();
        while(my $ref_select = $sqr_select->fetchrow_hashref())
        {
