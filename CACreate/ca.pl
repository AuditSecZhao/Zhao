...
if(system("openssl req -new -days 36500 -key user.key -out $user_name.csr -subj \"/C=CN/ST=Baolei/L=Baolei/O=Baolei/OU=Baolei/CN=$cert_name\" 1>/dev/null 2>&1") != 0)
{
...
if(system("openssl ca -startdate 130825000000Z -days 36500 -in $user_name.csr -out $user_name.crt -cert ca.crt -keyfile ca.key -batch 1>/dev/null 2>&1") != 0)
{
